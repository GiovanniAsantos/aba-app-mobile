...
import type { AllSignatureResponse, AllSignature } from '@/types/api';

type NavigationProp = NativeStackNavigationProp<RootStackParamList>;

const AllSignatureTab = () => {
    const { tokens } = useAuth();
    const navigation = useNavigation<NavigationProp>();
    const [signatures, setSignatures] = useState<AllSignature[]>([]);
    ...
    const statusOptions = [
        ...
        { label: 'Cadastrado aguardando Parte(s)', value: 'CADASTRADO_AGUARDADNDO_PARTES' },
        { label: 'Cadastrado aguardando ativação', value: 'CADASTRADO_AGUARDADNDO_ATIVACAO' },
        { label: 'Ativado aguardando assinaturas', value: 'ATIVADO_AGUARDANDO_ASSINATURAS' },
        { label: 'Desativado aguardando ativação', value: 'DESATIVADO_AGUARDANDO_ATIVACAO' },
        { label: 'Parcialmente Assinado', value: 'ASSINADO_PARCIALMENTE' },
        { label: 'Assinado por Todos', value: 'ASSINADO_POR_TODOS' },
    ...
    const getParticipantSignatures = async (page: number = 0) => {
        ...
        try {
            ...
            const url = `${apiSignatureUrlV1}/signatures/me/created-by?${params.toString()}`;
            ...
            const data: AllSignatureResponse = await response.json();
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            case 'CADASTRADO_AGUARDADNDO_PARTES':
                return { bg: '#e7f3ff', color: '#004085', label: 'Cadastrado aguardando Parte(s)' };
            case 'CADASTRADO_AGUARDADNDO_ATIVACAO':
                return { bg: '#e7f3ff', color: '#004085', label: 'Cadastrado aguardando ativação' };
            case 'ATIVADO_AGUARDANDO_ASSINATURAS':
                return { bg: '#fff3cd', color: '#856404', label: 'Ativado aguardando assinaturas' };
            case 'DESATIVADO_AGUARDANDO_ATIVACAO':
                return { bg: '#f8d7da', color: '#721c24', label: 'Desativado aguardando ativação' };
            case 'ASSINADO_PARCIALMENTE':
                return { bg: '#d1ecf1', color: '#0c5460', label: 'Parcialmente Assinado' };
            case 'ASSINADO_POR_TODOS':
                return { bg: '#d4edda', color: '#155724', label: 'Assinado por Todos' };
            case 'FINALIZADO':
                return { bg: '#d4edda', color: '#155724', label: 'Finalizado' };
            case 'CANCELADO':
            ...
            {!loading && filteredSignatures.length === 0 ? (
                ...
                <>
                    {filteredSignatures.map((item) => {
                        const statusStyle = getStatusStyle(item.crdInfo?.status || '');
                        return (
                            <View key={item.id} style={styles.assinaturaCard}>
                                ...
                                {/* Progresso */}
                                {item.percentage > 0 && item.crdInfo?.status !== 'CANCELADO' && (
                                ...
                                <View style={styles.assinaturaActions}>
                                    <View style={{ flex: 1 }}>
                                        <Button
                                            title={item.crdInfo?.status === 'FINALIZADO' ? 'Visualizar' : 'Gerenciar'}
                                            onPress={() => navigation.navigate('Signature', { signatureId: item.id.toString() })}
                                            variant={item.crdInfo?.status === 'FINALIZADO' ? 'secondary' : 'primary'}
                                        />
                                    </View>
                                </View>
                            </View>
...
export default AllSignatureTab