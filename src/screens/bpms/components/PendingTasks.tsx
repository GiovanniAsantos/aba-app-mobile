...
export const PendingTasks: React.FC<PendingTasksProps> = ({
  ...
  const loadActivities = async () => {
    ...
    setLoading(true);
    console.log(`🔄 Carregando atividades pendentes - Página: ${currentPage}`);
    ...
    try {
      ...
      // statusSla e outros filtros podem ser adicionados conforme necessário

      const response = await fetch(`${apiBpmsUrlV1}/tasks/me/pending?${params.toString()}`, {
        method: 'GET',
        headers: {
      ...
      console.log("🚀 ~ loadActivities ~ data:", data)
      ...
      Alert.alert('Erro', 'Não foi possível carregar as atividades pendentes');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <View style={styles.activityDates}>
        ...
        <View style={styles.dateItem}>
          ...
          <Text style={styles.dateText}>Por: {activity.createdBy}</Text>
        </View>
      </View>
    ...
    <ScrollView style={styles.activitiesList} showsVerticalScrollIndicator={false}>
      {loading ? (
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyStateText}>Nenhuma atividade pendente</Text>
        </View>
      ) : (
